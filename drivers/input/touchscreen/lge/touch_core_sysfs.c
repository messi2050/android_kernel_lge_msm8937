...
static char incoming_call_str[INCOMING_CALL_MAX][13] = {"IDLE", "RINGING", "OFFHOOK",
			"CDMA RINGING","CDMA OFFHOOK", "LTE RINGING", "LTE OFFHOOK"};
		...
		const char *buf, size_t count)
{
	...
	if (value >= INCOMING_CALL_IDLE && value < INCOMING_CALL_MAX ) {
